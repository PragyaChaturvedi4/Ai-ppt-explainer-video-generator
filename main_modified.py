...
def main():
    ...
    if not os.path.exists(face_image_path):
        print(f"ERROR: Cannot find face image '{face_image_path}'. Please upload a face image!")
        return
    ...
    for i, exp in enumerate(explanations):
        ...
        if os.path.exists(audio_file) and os.path.exists(image_file):
            render_video_part(image_file, audio_file, video_part)
    ...
    if video_parts:
        ...
        print("\nSUCCESS! Pipeline complete! Check output/final_video.mp4")
    else:
